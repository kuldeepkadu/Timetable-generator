<?php 
	...
	if(isset($_POST['submit']))
	{
		...
		$year = $_POST['year'];
		$sem = $_POST['sem'];
		$pract_code = $_POST['pract_code'];
		$pract_name = $_POST['pract_name'];
		
		$dept = stripcslashes($dept);
		$year = stripcslashes($year);
		$sem = stripcslashes($sem);
		$pract_code = stripcslashes($pract_code);
		$pract_name = stripcslashes($pract_name);
		
		$dept = mysql_real_escape_string($dept);
		$year = mysql_real_escape_string($year);
		$sem = mysql_real_escape_string($sem);
		$pract_code = mysql_real_escape_string($pract_code);
		$pract_name = mysql_real_escape_string($pract_name);

		$sql = "INSERT INTO course (dept,year,sem,sub_code,sub_name,status) VALUES ('$dept','$year','$sem','$pract_code','$pract_name',1)";
...
<head>
	...
	<script type="text/javascript">
		...
		function validate()
		{
			...
			var s3 = document.getElementById('sem_select').value;
			...
			if (true) {
					alert("Successfully Added Practicle");
					return true;
				}
				...
				<p class="blink">Fill Course Detail</p>
                   ...
                   <input type="hidden" id="hidden_year" name="hidden_year"  value="<?PHP echo trim($_POST['year']); ?>" />
						...
						<div>
							...
							<br>
							<br>
							pract_Code	<!--pattern="[0-9]{10}" pattern="[0-9]" -->
							<input type="pract_code" size="30" maxsize="200px" required   oninvalid="setCustomValidity('Invalid Practical Code')" name="pract_code" id="pract_code">
						
							<br>
							<br>
							pract_Name
							<input type="text" size="49" maxsize="200px" required name="pract_name" id="pract_name">
					...
					<br><br><br>
								<p>Note: To add Subject click on Options</a></p>
