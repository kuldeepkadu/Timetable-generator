...
<body>
	...
	<nav>
		...
		<?php
			...
			$query = "SELECT * FROM register WHERE post='Teacher'";
			$sqldata = mysqli_query($conn,$query) or die('Getting an error');

			echo "<table>";
			echo "<tr><th>&nbsp;SR</th><th>POST</th><th>USER NAME</th><th>PASSWORD</th><th>Delete</th><th>Update</th></tr>";
			$i=1;
			while($row = mysqli_fetch_array($sqldata,MYSQLI_ASSOC)) {
					...
					echo $row['pass1'];
					echo "</td><td>";
					echo "<a href='deleteTeacher.php?id=$row[id]' onclick='return checkDelete()' style='color: red;'>x</a>";
					echo "</td><td>";
					echo "<a href='editTeacher.php?id=$row[id]&uername=$row[username]&post=$row[post]'>Edit</a>";
					echo "</td></tr>";
					$i++;
			}
